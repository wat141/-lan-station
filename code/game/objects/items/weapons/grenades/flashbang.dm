...
//Checking for protections
	...
	if(iscarbon(M))
		...
		if(ishuman(C))
			...
			ear_safety = 0
			if(istype(H.ears, /obj/item/clothing/ears/earmuffs) || istype(H.head, /obj/item/clothing/head/helmet) || istype(H.head, /obj/item/clothing/head/beret/sec) || istype(H.head, /obj/item/clothing/head/soft/sec))
				ear_safety++
		if(ismonkey(C))
			ear_safety = 0
			takes_eye_damage = 1
